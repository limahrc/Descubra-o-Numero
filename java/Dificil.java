...
public class Dificil extends AppCompatActivity {
    ...
    private static final int N_TENTATIVAS = 12;      //for debugging purpose only. Default value is 12.
    private static final int MINIMAL_SCORE = 9000; //for debugging purpose only. Default value is 9000.
    private static final int NUMBER_RANGE = 1000;
    private static final int POINTS = 1000;
    private static final int REMOVE_POINTS = 500;
    private static final int DICA_ABSTRATA = 100;
    private static final int DICA_ESTATICA = 300;
    private static final int DICA_CONCRETA = 500;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MediaPlayer mInicio = MediaPlayer.create(this, R.raw.hard);
        mInicio.start();
        ...
        final MediaPlayer sErro = MediaPlayer.create(Dificil.this, R.raw.erro);
        final MediaPlayer sAcerto = MediaPlayer.create(Dificil.this, R.raw.respostacorreta);
        final MediaPlayer sPerda = MediaPlayer.create(Dificil.this, R.raw.gameover);
        final MediaPlayer sRestart = MediaPlayer.create(Dificil.this, R.raw.success1);
        final MediaPlayer sJerry = MediaPlayer.create(Dificil.this, R.raw.chaching);
    ...
    public void onClick_botaoVerificar(final Random randNum, final ArrayAdapter<String> adpNumJogados,
        ...
        if (campoNumero.getText().length() > 0) {
            ...
            if (numLido == numGerado) {
                ...
                pontuacao += POINTS;
                Toast.makeText(Dificil.this, "Acertou! Parabéns!", Toast.LENGTH_SHORT).show();
                isVictory = true;
                isLoss = false;
                clear(randNum, adpNumJogados, isVictory, isLoss);
            } else if (numLido > numGerado && tentativas >= 1) {
                sErro.start();
                Toast.makeText(Dificil.this, "Jogue um número menor!", Toast.LENGTH_SHORT).show();
                ...
                sErro.start();
                Toast.makeText(Dificil.this, "Jogue um número maior!", Toast.LENGTH_SHORT).show();
            ...
            if (tentativas == 0) {
                ...
                systemMessage.setNegativeButton("Não", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        startActivity(new Intent(Dificil.this, Registro.class));
                        finish();
                    }
                });
            ...
            Toast.makeText(Dificil.this, "O campo de número não deve ficar vazio", Toast.LENGTH_SHORT).show();
    }
        ...
        JerrySpeak.setItems(options, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                switch (which) {
                    case 0:
                        if (pontuacao >= MINIMAL_SCORE) {
                            Intent gotcha = new Intent(Dificil.this, Gotcha.class);
                            gotcha.putExtra("nome", nomeJogador);
                            startActivity(gotcha);
                            finish();
                            break;
                        } else
                            Toast.makeText(Dificil.this, "Desculpe, você ainda não tem pontos suficientes para avançar", Toast.LENGTH_SHORT).show();
                        break;
                    case 1:
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                switch (which) {
                                    case 0:
                                        ...
                                        } else Toast.makeText(Dificil.this, "Você não tem pontos suficientes para comprar esta dica", Toast.LENGTH_SHORT).show();
                                        break;
                                    case 1:
